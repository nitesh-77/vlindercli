...
**Status:** Partially superseded by ADR 064, 067, 068, 070

> **Note (post ADR 064-070):** The conversation storage model described here — JSON session files, `SubmissionId = commit SHA`, two commits per turn — is replaced by ADR 064 (git as agent protocol) and ADR 067 (one message, one node). Each agent message is now a git commit written by the DAG worker using plumbing commands; there are no JSON session files. The `vlinder timeline` CLI surface is replaced by ADR 068 (timeline is git passthrough). Agent identity in commits is defined by ADR 069. ADR 070 decouples the harness from the ConversationStore entirely — SubmissionId is now computed in-process using git's SHA-1 format, and the GitDagWorker is the sole writer to the conversations repo.
>
