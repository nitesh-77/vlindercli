...
use super::{
    SdkContract, ObjectStorageType, Registry, ServiceType, VectorMatch, VectorStorageType,
...
impl SdkContract for QueueBridge {
    fn kv_get(&self, path: &str) -> Result<Vec<u8>, String> {
        ...
        let hop = Hop { service: ServiceType::Kv, backend: backend.as_str().to_string(), operation: "get" };
        let state = self.current_state.read().unwrap().clone();
    ...
    fn kv_put(&self, path: &str, content: &str) -> Result<(), String> {
        ...
        let hop = Hop { service: ServiceType::Kv, backend: backend.as_str().to_string(), operation: "put" };
        let state = self.current_state.read().unwrap().clone();
    ...
    fn kv_list(&self, prefix: &str) -> Result<Vec<String>, String> {
        ...
        let hop = Hop { service: ServiceType::Kv, backend: backend.as_str().to_string(), operation: "list" };
    ...
    fn kv_delete(&self, path: &str) -> Result<bool, String> {
        ...
        let hop = Hop { service: ServiceType::Kv, backend: backend.as_str().to_string(), operation: "delete" };
    ...
    fn vector_store(&self, key: &str, vector: &[f32], metadata: &str) -> Result<(), String> {
        ...
        let hop = Hop { service: ServiceType::Vec, backend: backend.as_str().to_string(), operation: "store" };
    ...
    fn vector_search(&self, vector: &[f32], limit: u32) -> Result<Vec<VectorMatch>, String> {
        ...
        let hop = Hop { service: ServiceType::Vec, backend: backend.as_str().to_string(), operation: "search" };
    ...
    fn vector_delete(&self, key: &str) -> Result<bool, String> {
        ...
        let hop = Hop { service: ServiceType::Vec, backend: backend.as_str().to_string(), operation: "delete" };
    ...
    fn infer(&self, model: &str, prompt: &str, max_tokens: u32) -> Result<String, String> {
        ...
        let hop = Hop { service: ServiceType::Infer, backend: backend.clone(), operation: "run" };
    ...
    fn embed(&self, model: &str, text: &str) -> Result<Vec<f32>, String> {
        ...
        let hop = Hop { service: ServiceType::Embed, backend: backend.clone(), operation: "run" };
